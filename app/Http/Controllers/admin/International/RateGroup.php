...
use App\Models\InternationalRateGroup;
class RateGroup extends Controller
{
    public function index()
    {
        $data = [];
        $data['title'] = 'View International Rate Group';
        $data['Rategroup'] = InternationalRateGroup::all();
        return view('admin.International_rateGroup_master.view_rate_group',$data);
    }
    ...
    public function add_group()
    {
        ...
        return view('admin.International_rateGroup_master.add_rate_group',$data);
    }

    public function store_rate(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'name' => 'required|string|unique:tbl_international_rate_group,name|regex:/^[a-zA-Z\s]+$/',
        ]);
        if($validation->fails())
        {
            $responce = [
            ...
            try {
                DB::beginTransaction();
                    $user = InternationalRateGroup::create([
                        'name' => $request->name,
    ...
    public function edit_rate($id)
    {
        ...
        $data['editData'] = InternationalRateGroup::find($id);
        return view('admin.International_rateGroup_master.edit_rate_group',$data);
    }
    ...
    public function update_rate_group(Request $request)
    {
        ...
        if ($validation->passes()) {
            try {
                DB::beginTransaction();
                    $post = InternationalRateGroup::find($request->id);
                    $post->update([
                        'name' => $request->name,
                        'description' => $request->description]);
                    $post->save();
