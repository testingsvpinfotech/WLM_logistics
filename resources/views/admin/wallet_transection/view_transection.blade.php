                ...
                <div class="col-12 mt-3">
                    <div class="card">
                        <div class="card-header  justify-content-between align-items-center">
                            ...
                            <span style="float:right;"><a href="{{route('admin.add-toup-transaction')}}" class="btn btn-outline-primary">TopUp Wallet</a></span>
                        </div>
                        <div class="card-body">
                        <div class="col-12">
                                    <form action="{{ route('admin.view-wallet-transaction')}}" method="get">
                                        @csrf
                                        <div class="form-row">  
                                                ...
                                                <select name="customer_id" class="form-control rounded" id="customer_id">
                                                    ...
                                                    @foreach ($customer as $key => $val )
                                                      ...
                                                      @if (!empty($_GET['customer_id']))
                                                          @if ($_GET['customer_id'] == $val->id)
                                                             {{'selected'}}
                                                          @endif
                                                      @endif
                                                      >{{$val->personal_name.' '.$val->surname.' ('.$val->company_name.')'}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="form-group col-md-3">
                                            ...
                                            <a href="{{route('admin.view-wallet-transaction')}}" class="btn btn-outline-danger mt-4"><i class="fa fa-refresh" aria-hidden="true"></i></a>
                                            </div>
                                        </div>
                                    </form>
                                </div>
